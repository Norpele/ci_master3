...
class Personal extends CI_Controller
{
    ...
    function import_excel()
    {
        if (isset($_FILES["file"])) {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                if ($rowData) {
                    $tglin = $this->tgl_excel(floatval($rowData[0][13]));
                    $tglout = $this->tgl_excel(floatval($rowData[0][14]));
                    $tglgraduate = $this->tgl_excel(floatval($rowData[0][4]));
    ...
    public function exportExcel(){
        $sql = "SELECT branchName, employeeCode,bankName,bpjsName, employeeName from employee 
